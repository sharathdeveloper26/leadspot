          ...
          <div className="p-8 space-y-8">
            ...
            <div className="space-y-6">
                  ...
                  {((lead.designation && lead.designation !== "Unknown") || (lead.location && lead.location !== "Unknown") || lead.linkedin || (lead.truecallerName && lead.truecallerName !== "Unknown")) && (
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-slate-700 font-medium bg-white/60 p-2 rounded-xl border border-white shadow-sm">
                      ...
                      {lead.truecallerName && lead.truecallerName !== "Unknown" && (
                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-[#9face6]/10 text-[#7a8ece] rounded-lg text-xs font-bold border border-[#9face6]/30 shadow-sm" title="Verified Name">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* ✨ NEW: TRUECALLER ENRICHMENT BADGE ✨ */}
                {(lead.truecallerName && lead.truecallerName !== "Unknown") && (
                  <div className="bg-gradient-to-r from-blue-50/80 to-indigo-50/30 backdrop-blur-sm p-5 rounded-2xl border border-blue-100 shadow-[0_4px_20px_rgba(0,0,0,0.02)] flex items-center gap-4 md:col-span-2">
                    ...
                    <div>
                      <p className="text-[10px] font-extrabold text-blue-400 uppercase tracking-widest mb-1">Verified via Truecaller</p>
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-slate-800 font-bold">{lead.truecallerName}</p>
                        {lead.truecallerBusiness && lead.truecallerBusiness !== "Unknown" && (
                          <span className="px-2 py-0.5 bg-blue-600 text-white text-[9px] font-black uppercase tracking-widest rounded shadow-sm">
                            🏢 Business
                          </span>
                        )}
                      </div>
                ...
                {user?.role === 'client_admin' && (
                  <div className="bg-white/60 backdrop-blur-sm p-5 rounded-2xl border border-white shadow-[0_4px_20px_rgba(0,0,0,0.02)] flex items-center gap-4 md:col-span-2 mt-2">
              ...
              {/* ✨ UPGRADED MARKETING ATTRIBUTION ✨ */}
              {(lead.adName || lead.campaignName || lead.formId || lead.adId || lead.campaignId || lead.utm_source || lead.utm_medium || lead.utm_campaign) && (
                <div className="bg-white/60 backdrop-blur-sm p-6 rounded-2xl border border-white shadow-[0_4px_20px_rgba(0,0,0,0.02)] mt-6">
                  <div className="flex items-center gap-3 mb-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-white/80 p-5 rounded-xl border border-slate-100">
                    ...
                    {(lead.utm_source || lead.utm_medium || lead.utm_campaign) && (
                      <div className="col-span-1 sm:col-span-2 pt-5 border-t border-slate-200/60">
                        <span className="block text-[10px] font-extrabold text-indigo-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                        ...
                        <div className="flex flex-wrap gap-3">
                          {lead.utm_source && (
                            <div className="bg-white border border-slate-200 px-3 py-2 rounded-xl flex flex-col shadow-sm min-w-[120px]">
                              <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider mb-0.5">UTM Source</span>
                              <span className="text-xs font-bold text-slate-700">{lead.utm_source}</span>
                            </div>
                          )}
                          {lead.utm_medium && (
                            <div className="bg-white border border-slate-200 px-3 py-2 rounded-xl flex flex-col shadow-sm min-w-[120px]">
                              <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider mb-0.5">UTM Medium</span>
                              <span className="text-xs font-bold text-slate-700">{lead.utm_medium}</span>
                            </div>
                          )}
                          {lead.utm_campaign && (
                            <div className="bg-white border border-slate-200 px-3 py-2 rounded-xl flex flex-col shadow-sm min-w-[120px]">
                              <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider mb-0.5">UTM Campaign</span>
                              <span className="text-xs font-bold text-slate-700">{lead.utm_campaign}</span>
                            </div>
                          )}
                        </div>
                      </div>
